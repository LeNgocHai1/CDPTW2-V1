...
@section('content')
    <div class="container jumbotron   border border-success">
            ...
            <tbody>
                @foreach ($products as $product)
                    <tr>
                        ...
                        <td>
                            <a class="button btn btn-success" href="{{ route('prodetail.edit', $product->id) }}"><i
                                    class="fas fa-tools"></i> Sửa</a>
                            <a class="button btn btn-info" href="{{ route('prodetail.show', $product->id) }}"><i
                                    class="fas fa-tools"></i> Xem</a>
                            <form class="d-inline-block " action="{{ route('prodetail.destroy', $product->id) }}"
                                method="post">
                                {{ csrf_field() }}
                                @method('DELETE')
                                <button type="submit" onclick="return confirm('Bạn có chắc chắn muốn xóa?')"
                                    class="button btn btn-danger">Xóa</button>
                            </form>
        ...
        <p class="d-flex justify-content-end">
            <a class="btn btn-info btn-sm fa fa-plus" href="{{ route('prodetail.create') }}">Thêm chi tiết cho Sản
                phẩm</a>
        </p>
    </div>
@endsection
